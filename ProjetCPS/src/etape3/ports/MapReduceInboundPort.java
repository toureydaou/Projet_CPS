...
public class MapReduceInboundPort extends MapReduceSyncInboundPort implements MapReduceCI {
	...
	public <R extends Serializable, I extends MapReduceResultReceptionCI> void map(String computationURI,
		...
		this.getOwner().runTask(owner -> {
			try {
				((NodeBCM) owner).map(computationURI, selector, processor);
			} catch (Exception e) {
	...
	public <A extends Serializable, R, I extends MapReduceResultReceptionCI> void reduce(String computationURI,
		...
		this.getOwner().runTask(owner -> {
			try {
				((NodeBCM) owner).reduce(computationURI, reductor, combinator, identityAcc, currentAcc, callerNode);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
